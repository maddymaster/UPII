...
7.  **Input**: User types query in overlay.
8.  **Output**: Answer rendered in HTML.

## 7. Global Access Architecture (Overlay)
**Goal**: Instant (<300ms) availability via Cmd+K.

- **Stack**:
    - **UI**: `pywebview` (Lightweight WebKit wrapper, HTML/CSS/JS frontend).
    - **Backend**: `FastAPI` (headless) or direct Python binding.
    - **Trigger**: `pynput` global hotkey listener (Daemonized).

- **Flow**:
    1.  `upii-server` starts in background (hidden).
    2.  `Cmd+K` detected.
    3.  `pywebview` window "shows" (un-hides) instantly.
    4.  User Query -> `ContextRehydrator` -> `LocalLLM`.
    5.  Response rendered in Glassmorphism UI.
